...
async function processAccidentReport(documentId: string, data: any) {
  try {
    ...
    const geburtsdatumMatch = data.text.match(/Geburtsdatum[^:]*:\s*(\d{1,2}[.\s/-]\d{1,2}[.\s/-]\d{4})/i);
    const unfallDatumMatch = data.text.match(/(?:Unfall|Schaden)[^:]*datum[^:]*:\s*(\d{1,2}[.\s/-]\d{1,2}[.\s/-]\d{4})/i);

    // Formatiere und validiere Daten
    let formattedGeburtsdatum = geburtsdatumMatch ? formatDate(geburtsdatumMatch[1].trim()) : null;
    let formattedUnfallDatum = unfallDatumMatch ? formatDate(unfallDatumMatch[1].trim()) : formatDate(new Date().toLocaleDateString('de-CH'));
    ...
    const schadenSection = extractedText.match(/##\s*4\.\s*Schaden\n([\s\S]*?)(?=##|$)/i)?.[1] || '';
    const datumMatch = schadenSection.match(/Datum[^:]*:\s*\*?\*?(\d{1,2}[.\s]\d{1,2}[.\s]\d{4})\*?\*?/i) ||
                      extractedText.match(/(?:Tag\/Monat\/Jahr|Datum|Schaden-?\s*datum)[^:]*:\s*(\d{1,2}[.\s]\d{1,2}[.\s]\d{4})/i);
    ...
    const unfallBeschreibung = extractedText.match(/(?:Unfallhergang|Beschreibung)[^:]*:\s*([^\n]+)/i)?.[1]?.trim() || '';
    const verletzungArt = extractedText.match(/Art\sder\sVerletzung[^:]*:\s*([^\n]+)/i)?.[1]?.trim() || '';
    const verletzungKoerperteil = extractedText.match(/Verletzter\sKörperteil[^:]*:\s*([^\n]+)/i)?.[1]?.trim() || '';
    ...
    console.error('Document processing error:', error);
    await updateDocumentStatus(documentId, 'failed', `Fehler bei der Verarbeitung: ${error.message}`);
    throw error;
  }
}
...
async function processDamageReport(documentId: string, data: any) {
  ...
  const versicherungsnummer = extractedText.match(/Versicherungsnummer[^:]*:\s*(\d+)/i)?.[1] || null;
  const name = extractedText.match(/Name[^:]*:\s*([^\n]+)/i)?.[1]?.trim() || '';
  const adresse = extractedText.match(/Adresse[^:]*:\s*([^\n]+)/i)?.[1]?.trim() || '';
  const schadenDatum = extractedText.match(/Schadensdatum[^:]*:\s*(\d{1,2}[.\s]\d{1,2}[.\s]\d{4})/i)?.[1]?.replace(/\s+/g, '.') || '';
  const schadenOrt = extractedText.match(/Schadenort[^:]*:\s*([^\n]+)/i)?.[1]?.trim() || '';
  const schadenBeschreibung = extractedText.match(/Schadenbeschreibung[^:]*:\s*([^\n]+)/i)?.[1]?.trim() || '';
...
async function processContractChange(documentId: string, data: any) {
  ...
  const aenderungBeschreibung = extractedText.match(/Beschreibung[^:]*:\s*([^\n]+)/i)?.[1]?.trim() || '';
...
// Hilfsfunktion für die Anzeige im Frontend
