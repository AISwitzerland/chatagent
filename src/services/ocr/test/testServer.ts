...
import { OCRResult } from '../types/ocrTypes.js';
...
if (!process.env.OPENAI_API_KEY) {
  throw new Error('OPENAI_API_KEY ist nicht in der .env Datei konfiguriert');
}
...
app.post('/test/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    // OCR mit GPT-Vision
    const ocrResult: OCRResult = await gptVisionProcessor.processImage(
      base64Image,
